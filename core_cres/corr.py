...
import numpy as np
from .cres_utils import bilinear_sampler, coords_grid, manual_pad
...
class AGCL:
    ...
    def corr_att_offset(
        ...
        for i in range(len(psize_list)):
            ...
            y_grid, x_grid = torch.tensor(
                y_grid, device=self.fmap1.device
            ), torch.tensor(x_grid, device=self.fmap1.device)
            offsets = torch.stack((x_grid, y_grid)).reshape(2, -1).permute(1, 0)
            offsets = (
                offsets.unsqueeze(0).unsqueeze(2).unsqueeze(3).repeat(N, 1, H, W, 1)
            )
            offsets = offsets + extra_offset

            coords = self.coords + flow
            coords = coords.permute(0, 2, 3, 1).unsqueeze(1) + offsets
            coords = coords.reshape(N, -1, W, 2)

            right_feature = bilinear_sampler(right_feature, coords)
            right_feature = right_feature.reshape(N, C, -1, H, W)
            left_feature = left_feature.unsqueeze(2).repeat(
                1, 1, right_feature.shape[2], 1, 1
            )
