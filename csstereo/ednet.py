...
class EDNet(nn.Module):
    ...
    def __init__(self, in_shape, in_channels, out_channels, out_scale):
        ...
        self.d3 = Decoder(
            shape[2], nf[3], nf[2], out_channels, nf[2] + out_channels, out_scale
        )
        self.d2 = Decoder(
            shape[1], nf[2], nf[1], out_channels, nf[1] + out_channels, out_scale
        )
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_shape, out_shape, in_channels, out_channels, ksize=3):
        ...
        self.conv = Conv2dAP(
            in_shape, out_shape, in_channels, out_channels, ksize, 2, False
        )
        self.convbn = nn.BatchNorm2d(out_channels)
        self.convb = Conv2dAP(
            out_shape, out_shape, out_channels, out_channels, ksize, 1, False
        )
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.conv = Conv2dAP(
            out_shape, out_shape, in_channels, out_channels, ksize, 1, False
        )
        self.convbn = nn.BatchNorm2d(out_channels)
        self.iconv = Conv2dAP(
            out_shape,
        ...
        if out_scale > 0:
            self.out = Conv2dAP(
                out_shape, out_shape, out_channels, final_out_channels, ksize, 1, True
            )
    ...
    def forward(self, x, skip):
        upconv = F.elu(
            self.convbn(self.conv(F.interpolate(x, self.out_shape, mode="bilinear")))
        )
...
class Conv2dAP(nn.Module):
    def __init__(self, in_shape, out_shape, in_c, out_c, ksize, stride, bias):
        ...
        pad = [0, 0]
        pad[0] = int(
            math.ceil((stride[0] * (out_shape[0] - 1) - in_shape[0] + ksize[0]) / 2)
        )
        pad[1] = int(
            math.ceil((stride[1] * (out_shape[1] - 1) - in_shape[1] + ksize[1]) / 2)
        )
    ...
    def forward(self, x):
        xs = x.size()
        assert xs[1] == self.in_c and xs[2] == self.in_s[0] and xs[3] == self.in_s[1]
        y = self.layer(x)
        ys = y.size()
