...
class Rendering(nn.Module):
    ...
    def get_coordgrid(self, x):
        ...
        b, _, h, w = x.size()
        grid_h = (
            torch.linspace(0.0, w - 1, w, device=x.device, dtype=x.dtype)
            .view(1, 1, 1, w)
            .expand(b, 1, h, w)
        )
        grid_v = (
            torch.linspace(0.0, h - 1, h, device=x.device, dtype=x.dtype)
            .view(1, 1, h, 1)
            .expand(b, 1, h, w)
        )
    ...
    def median_filter_2d(self, x, kernel_size=3):
        b, c, h, w = x.size()
        x_unfold = F.unfold(
            x.float(), kernel_size=kernel_size, padding=kernel_size // 2
        )
    ...
    def rendering(self, x, pixel_coords_2d, sort_by=None, return_idx=False):
        ...
        if sort_by is not None:
            ...
            point_values_norm = (
                1.0 / (point_values[..., [-1]] + 1e-8)
                if point_values.shape[-1] > 1
                else point_values
            )
        ...
        out, arg_max = scatter_max(
            point_values_norm, linear_idx, dim=-2, out=image_flat
        )
        ...
        max_vals = torch.where(
            arg_valid_mask, max_vals, torch.ones_like(max_vals) * 1e8
        )
    ...
    def forward(self, x, flow_in, return_mask=False):
        ...
        image, arg_max = self.rendering(
            x, pixel_coords_2d, sort_by=sort_by, return_idx=True
        )
        if not return_mask:
            return image
        else:
            mask = torch.zeros_like(arg_max, requires_grad=False).scatter_(
                1, arg_max * (arg_max >= 0) * (arg_max < (H * W)), 1
            )
            mask[..., 0, :] = 0.0
...
class Fake_Flip:
    ...
    def generate_fake_flip(self, inputs: dict, model):
        ...
        def get_fake_img(img_l_input, img_r_input, flip=False):
            ...
            disp = (
                torch.flip(outputs_h[("disp", max_i)], [3])
                if flip
                else outputs_h[("disp", max_i)]
            )
            if flip:
                fake_image, valid_mask = self._rendering(
                    right_aug_wider, -disp, return_mask=True
                )
                fake_image = pad_zeros(fake_image, right_aug_wider)
                fake_image = fake_image[..., : -inputs["wider_pixels"][0]]
            else:
                fake_image, valid_mask = self._rendering(
                    left_aug_wider, -disp, return_mask=True
                )
        ...
        if random.random() > 0.5:
            ...
            fake_image = get_fake_img(
                torch.flip(right_raw_wider, [3]),
                torch.flip(left_raw_wider, [3]),
                flip=True,
            )
            fake_image_no_flip = inputs["right"]
        else:
        ...
        inputs_fake[("color", 0, 0)] = torch.where(
            flip_mask, inputs[("color", "s", 0)], inputs[("color", 0, 0)]
        )
        inputs_fake[("color", "s", 0)] = torch.where(
            flip_mask, inputs[("color", 0, 0)], inputs[("color", "s", 0)]
        )
...
def warp(x, disp, padding_mode="border", mode="bilinear"):
    """
